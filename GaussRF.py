...
if __name__ == "__main__":
    ...
    N = 20 # order of the KL expansion
    M = 100 # M quadrature points
    def exp_cov(t, s):
        return np.minimum(s,t)
    a, b = 0., 1.
    #phi, L = GaussF_1D(N, M, a, b, exp_cov).eigens()
    #plt.plot(X)
    #plt.show()
    #L_ref = [ 1./((k + 0.5)*np.pi)**2 for k in range(0,20)]
    #print(L[:10], L_ref[:10])

    def K2D(x,y):
        return np.exp(-np.linalg.norm(x - y, 1, axis = 1))
    
    X = GaussF_KL1D(N, 30, 0., 1., exp_cov,"KL_gaussleg")
    print(repr(X))
    phi, L = X.eigens()
    print(L[:10])
